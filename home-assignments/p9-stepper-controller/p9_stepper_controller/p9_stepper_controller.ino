/*
  ...
  NOTE: Stepper driver pins were shifted from the sheet's 8-11 to 7-10 because
  the kit's actual OLED is the 7-pin SPI SSD1306, and hardware SPI uses D11
  (MOSI) / D13 (SCK) on the Uno - those must stay free.
*/
...
const int STEPS_PER_REV = 2048; // half-step mode effective count for 28BYJ-48
...
void loop() {
  ...
  // edge. That way each click - lock down OR pop back up - produces one step.
  if (cw != lastCW) {
